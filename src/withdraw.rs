...
impl<const H: usize> ConstraintSynthesizer<Fr> for WithdrawCircuit<H> {
   fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
       ...
       let recipient_f_in =
           FpVar::<Fr>::new_input(cs.clone(), || self.recipient_f.ok_or(SynthesisError::AssignmentMissing))?;
       ...
       let recipient_square =
           FpVar::<Fr>::new_witness(cs.clone(), || self.recipient_square.ok_or(SynthesisError::AssignmentMissing))?;
...
mod tests {
   ...
   fn withdraw_circuit_groth16() {
       ...
       let proof: ark_groth16::Proof<ark_ec::bn::Bn<ark_bn254::Config>> = Groth16::<Bn254>::prove(&pk, circ, &mut rng).unwrap();
